...
class CompoundGNNModel(nn.Layer):
    ...
    | CompoundGNNModel, implementation of the variant GNN models in paper
        ``GraphKM: Predicting substrate-enzyme KM value with graph neural networks``.
...
class KMModel(nn.Layer):
    """
    | KMModel, implementation of the network architecture in GraphKM.
...
class KMModelCriterion(nn.Layer):
    """
    | KMModelCriterion, implementation of MSE loss for GraphKM model.

    Public Functions:
        - ``forward``: forward function.
    """
    def __init__(self):
        super(KMModelCriterion, self).__init__()

    def forward(self, pred, label):
        """Forward function.

        Args:
            pred (Tensor): KM value predictions, i.e. output from KMModel.
            label (Tensor): KM label.
        """
