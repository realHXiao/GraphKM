...
from src.data_gen import KMDataset  

import pgl
import paddle
import numpy as np
from src.model import KMModel, KMModelCriterion
...
def main(args): 
    ...
    train_dataset = KMDataset(train_data, max_protein_len=max_protein_len)
    test_dataset = KMDataset(test_data, max_protein_len=max_protein_len)
    ...
    for epoch_id in range(1, args.max_epoch + 1):
        ...
        if test_rmse < best_rmse:
            ...
            # save KM values and predicted KM values. 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lr_min", type=float, default=0, help = 'Optional! Default 0. Enable after --lr_CosineAnnealingDecay specified.')
    parser.add_argument("--lr_max", type=float, default=0.0005, help = 'Optional! Default 0.0005. Enable after --lr_CosineAnnealingDecay specified.')
